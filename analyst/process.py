...
# from utils.plot_util import draw_fun


def basic_process_pair(df1, df2):
    """
    :param df1: 次主力合约
    :param df2: 主力合约
    :return: df: 处理好的数据 trend_spot: 只包含拐点的数据
    """

    # 对合约1进行处理
    df1['datetime'] = pd.to_datetime(df1['datetime'])
    df1.drop('datetime_nano', axis=1, inplace=True)
    df1.columns = ['datetime', 'open1', 'high1', 'low1', 'close1', 'volume1', 'open_oi1', 'close_oi1']
    df1 = df1[['datetime', 'open1', 'close1', 'volume1']]

    # 对合约2进行处理
    df2['datetime'] = pd.to_datetime(df2['datetime'])
    df2.drop('datetime_nano', axis=1, inplace=True)
    df2.columns = ['datetime', 'open2', 'high2', 'low2', 'close2', 'volume2', 'open_oi2', 'close_oi2']
    df2 = df2[['datetime', 'open2', 'close2', 'volume2']]
    ...
    df = df.loc[df['hour'] < 21]

    # 价差计算
    df['open'] = df['open1'] - df['open2']
    df['close'] = df['close1'] - df['close2']
    df['volume'] = df['volume1'] + df2['volume2']
    # 计算总体的成交量
    df['vol_cumsum'] = df['volume'].cumsum()
...
def basic_process_oneside(df):
    ...
    df = df[['datetime', 'open', 'close', 'volume']]
...
class TrendAnalyst:
    ...
    def get_stats_data(self,
        ...
        # 对K值进行遍历
        for i, j in zip(k1_list, k2_list):
            print(i, j)
...
if __name__ == '__main__':
    ...
    main = pd.read_csv('../data/raw/%s/%s.csv' % (target, main_name))
    sub_main = pd.read_csv('../data/raw/%s/%s.csv' % (target, sub_main_name))

    # 跨品种or跨期价差分析 sub_main - main
    # spread_data, spread_spot = basic_process_pair(sub_main, main)

    # 单边处理
    spread_data, spread_spot = basic_process_oneside(main)
    ...
    # plot_trend_reverse(filter_spot, k1, k2)

    # draw_fun(spread_data, spread_spot, title)
